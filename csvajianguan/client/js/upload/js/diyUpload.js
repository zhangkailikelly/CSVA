...
(function( $ ) {
    ...
    $.fn.extend({
		...
		*/
        diyUpload:function( opt, serverCallBack ) {
			...
			//迭代出默认配置
			$.each( getOption( '#'+$fileInputId,opt.imgCount ),function( key, value ){
     ...
     webUploader.model = opt.model; //挂载一个model属性
	...
	//Web Uploader默认配置;
	function getOption(objId, imgCount) {
    if(imgCount && imgCount == 0){
      imgCount = -1 //当图片已经到上限的时候，需要设置为-1(具体原因不详)
    }
		/*
	...
	function createBox( $fileInput, file, webUploader ) {
		...
		var li = '<li id="fileBox_'+file_id+'" class="diyUploadHover"> \
					...
					<input type="hidden" name="'+webUploader.model+'[picture]" value="'+file.name+'"> \
				</li>';
